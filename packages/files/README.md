...
`@acme/files` is intended to be consumed as an internal workspace dependency in
this monorepo.
...
import convexFilesControl from "@acme/files/convex.config";
...
show one way to integrate it into your own Convex app.
...
```ts
import { buildDownloadUrl } from "@acme/files";
...
required). If you enable `requireAccessKey` on the HTTP route, shareable links
will still require `checkDownloadRequest` to return an access key.
...
Use `cleanUp.cleanupExpired` to delete expired uploads, grants, and files. A
common pattern is to wrap this in a mutation and run it from a cron job.
...
```ts
import { FilesControl } from "@acme/files";
