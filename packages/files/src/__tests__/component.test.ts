...
async function createRegisteredFile(
  t: TestContext,
...
describe("download", () => {
  ...
  test("consumeDownloadGrantForUrl handles statuses", async () => {
    ...
    const fileMissing = await t.mutation(
      api.download.consumeDownloadGrantForUrl,
      {
    ...
    const blankAccess = await t.mutation(
      api.download.consumeDownloadGrantForUrl,
      {
    ...
    const wrongAccess = await t.mutation(
      api.download.consumeDownloadGrantForUrl,
      {
    ...
    const okPassword = await t.mutation(
      api.download.consumeDownloadGrantForUrl,
      {
        downloadToken: passwordGrant.downloadToken,
        accessKey: "pw",
    ...
    const expiredFile = await t.mutation(
      api.download.consumeDownloadGrantForUrl,
      {
    ...
    const missingBlob = await t.mutation(
      api.download.consumeDownloadGrantForUrl,
      {
