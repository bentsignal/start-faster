...
describe("FilesControl class and clientApi", () => {
  ...
  test("FilesControl methods pass r2 config and throw when missing", async () => {
    ...
    expect(runAction).toHaveBeenCalledWith(component.upload.computeR2Metadata, {
      storageId: "storage",
      r2Config,
    });
    ...
    expect(runAction).toHaveBeenCalledWith(component.transfer.transferFile, {
      storageId: "storage",
      targetProvider: "convex",
      r2Config,
    });
  ...
  test("FilesControl handles missing process global", async () => {
    ...
    await expect(
      ...
      "R2 configuration is missing required fields for R2 uploads",
    );
  });
