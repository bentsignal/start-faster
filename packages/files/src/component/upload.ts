...
export const finalizeUpload = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (pendingUpload.storageId && pendingUpload.storageId !== args.storageId) {
    ...
    if (
      ...
      pendingVirtualPath !== normalizedVirtualPath
    ) {
...
async function registerFileCore(
  ...
  const [existingRegistration, systemFile, existingVirtualPath] =
    await Promise.all([
      ...
      virtualPath
        ? findFileByVirtualPath(ctx, virtualPath)
  ...
  const metadata =
    args.metadata ?? (args.storageProvider === "convex" ? systemFile : null);
