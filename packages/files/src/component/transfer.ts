  ...
  handler: async (ctx, args): Promise<TransferResult> => {
    ...
    if (resolvedVirtualPath) {
      const existing = await ctx.runQuery(
        internal.transfer.getFileByVirtualPathForTransfer,
        {
    ...
    const needsR2 =
      file.storageProvider === "r2" || args.targetProvider === "r2";
    const r2Config = needsR2
      ? requireR2Config(args.r2Config, "R2 transfers")
      : null;
